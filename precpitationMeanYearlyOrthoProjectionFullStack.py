...
for index in range(0, len(time)): 
    ...
    yearlyMean[finalIndex-1,:,:] = np.add(yearlyMean[finalIndex-1,:,:],totalPrecip)
    ...
    if currentDate.month == 12:
        ...
        yearlyMean[yearlyMean == -9.96921e+36] = None
        fig.suptitle('Global Yearly Mean Precipitation in mm', y=0.15,fontsize=18)
        ...
        #fig.tight_layout()
        plt.savefig('D:/Ankit/big data/project/files/image/meanPrecipitationYearlyReduced/'+ str(finalIndex) + '.png') #str(new_date.date())+ str(new_date.time()).replace(':','')
        plt.close()
